...
import { toReadableStream, sseHeaders } from "@/lib/stream";
...
export async function POST(req: NextRequest) {
  ...
  async function* run() {
    try {
      ...
      yield JSON.stringify({ type: "status", msg: "connected" });
      ...
      yield JSON.stringify({ type: "status", msg: "retrieval complete", contextUsed: used, relatedConcepts });
      ...
      yield JSON.stringify({ type: "status", msg: "policy surface ready" });
  ...
  return new Response(toReadableStream(run()), { headers: sseHeaders() });
}
